#!/usr/bin/env python3
"""
South Carolina Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://businessfilings.sc.gov/BusinessFiling/Entity/Search
  - Main:       https://sos.sc.gov/business-filing
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("south_carolina_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/south_carolina_businesses.json",
    "state_file":           "data/south_carolina_scraper_state.json",
...
class SouthCarolinaScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new South Carolina businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique South Carolina businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("SOUTH CAROLINA SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "SC"])
        ...
        logger.info(f"  SC total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ South Carolina scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting South Carolina Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="South Carolina Business Scraper - 24h Scheduler")
    ...
    sc = SouthCarolinaScheduledScraper(cfg)
