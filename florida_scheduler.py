...
CONFIG = {
    "data_file":            "data/florida_businesses.json",
...
def _zip():
    return str(random.randint(32004, 34997)).zfill(5)
...
class FloridaScheduledScraper:
    ...
    def run_once(self):
        ...
        state_count = len([b for b in merged if b.get("state") == "FL"])
        ...
        logger.info(f"  FL total         : {state_count}")
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Florida Business Scraper - 24h Scheduler")
    ...
    sc = FloridaScheduledScraper(cfg)
