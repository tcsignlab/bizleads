#!/usr/bin/env python3
"""
New Hampshire Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://quickstart.sos.nh.gov/online/Account/LandingPage
  - Main:       https://www.sos.nh.gov/corporations
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("new_hampshire_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/new_hampshire_businesses.json",
    "state_file":           "data/new_hampshire_scraper_state.json",
...
class NewHampshireScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new New Hampshire businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique New Hampshire businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("NEW HAMPSHIRE SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "NH"])
        ...
        logger.info(f"  NH total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ New Hampshire scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting New Hampshire Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="New Hampshire Business Scraper - 24h Scheduler")
    ...
    sc = NewHampshireScheduledScraper(cfg)
