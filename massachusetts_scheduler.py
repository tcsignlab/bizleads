#!/usr/bin/env python3
"""
Massachusetts Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://corp.sec.state.ma.us/corpweb/CorpSearch/CorpSearch.aspx
  - Main:       https://www.sec.state.ma.us/divisions/corporations/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("massachusetts_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/massachusetts_businesses.json",
    "state_file":           "data/massachusetts_scraper_state.json",
...
class MassachusettsScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Massachusetts businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Massachusetts businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("MASSACHUSETTS SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "MA"])
        ...
        logger.info(f"  MA total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Massachusetts scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Massachusetts Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Massachusetts Business Scraper - 24h Scheduler")
    ...
    sc = MassachusettsScheduledScraper(cfg)
