#!/usr/bin/env python3
"""
Connecticut Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://service.ct.gov/business/s/onlinebusinesssearch
  - Main:       https://portal.ct.gov/sots/business-services/commercial-recording-division
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("connecticut_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/connecticut_businesses.json",
    "state_file":           "data/connecticut_scraper_state.json",
...
class ConnecticutScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Connecticut businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Connecticut businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("CONNECTICUT SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "CT"])
        ...
        logger.info(f"  CT total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Connecticut scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Connecticut Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Connecticut Business Scraper - 24h Scheduler")
    ...
    sc = ConnecticutScheduledScraper(cfg)
