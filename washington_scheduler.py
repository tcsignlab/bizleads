#!/usr/bin/env python3
"""
Washington Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://ccfs.sos.wa.gov/#/
  - Main:       https://www.sos.wa.gov/corps/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("washington_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/washington_businesses.json",
    "state_file":           "data/washington_scraper_state.json",
...
class WashingtonScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Washington businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Washington businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("WASHINGTON SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "WA"])
        ...
        logger.info(f"  WA total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Washington scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Washington Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Washington Business Scraper - 24h Scheduler")
    ...
    sc = WashingtonScheduledScraper(cfg)
