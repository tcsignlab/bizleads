#!/usr/bin/env python3
"""
Iowa Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://sos.iowa.gov/search/business/
  - Main:       https://sos.iowa.gov/businesses.html
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("iowa_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/iowa_businesses.json",
    "state_file":           "data/iowa_scraper_state.json",
...
class IowaScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Iowa businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Iowa businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("IOWA SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "IA"])
        ...
        logger.info(f"  IA total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Iowa scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Iowa Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Iowa Business Scraper - 24h Scheduler")
