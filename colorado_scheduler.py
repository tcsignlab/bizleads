#!/usr/bin/env python3
"""
Colorado Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://www.sos.state.co.us/biz/BusinessEntityCriteriaExt.do
  - Main:       https://www.sos.state.co.us/pubs/business/main.html
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("colorado_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/colorado_businesses.json",
    "state_file":           "data/colorado_scraper_state.json",
...
class ColoradoScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Colorado businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Colorado businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("COLORADO SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "CO"])
        ...
        logger.info(f"  CO total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Colorado scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Colorado Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Colorado Business Scraper - 24h Scheduler")
    ...
    sc = ColoradoScheduledScraper(cfg)
