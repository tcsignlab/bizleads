#!/usr/bin/env python3
"""
Arizona Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://ecorp.azcc.gov/EntitySearch/Index
  - Main:       https://azsos.gov/business
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("arizona_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/arizona_businesses.json",
    "state_file":           "data/arizona_scraper_state.json",
...
class ArizonaScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Arizona businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Arizona businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("ARIZONA SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "AZ"])
        ...
        logger.info(f"  AZ total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Arizona scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Arizona Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Arizona Business Scraper - 24h Scheduler")
    ...
    sc = ArizonaScheduledScraper(cfg)
