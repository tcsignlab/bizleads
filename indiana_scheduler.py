#!/usr/bin/env python3
"""
Indiana Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://bsd.sos.in.gov/publicbusinesssearch
  - Main:       https://www.in.gov/sos/business/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("indiana_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/indiana_businesses.json",
    "state_file":           "data/indiana_scraper_state.json",
...
class IndianaScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Indiana businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Indiana businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("INDIANA SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "IN"])
        ...
        logger.info(f"  IN total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Indiana scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Indiana Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Indiana Business Scraper - 24h Scheduler")
    ...
    sc = IndianaScheduledScraper(cfg)
