#!/usr/bin/env python3
"""
Maryland Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://egov.maryland.gov/BusinessExpress/EntitySearch
  - Main:       https://dat.maryland.gov/Pages/sdat.aspx
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("maryland_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/maryland_businesses.json",
    "state_file":           "data/maryland_scraper_state.json",
...
class MarylandScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Maryland businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Maryland businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("MARYLAND SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "MD"])
        ...
        logger.info(f"  MD total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Maryland scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Maryland Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Maryland Business Scraper - 24h Scheduler")
    ...
    sc = MarylandScheduledScraper(cfg)
