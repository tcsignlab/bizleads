#!/usr/bin/env python3
"""
South Dakota Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://sosenterprise.sd.gov/BusinessServices/Business/FilingSearch.aspx
  - Main:       https://sdsos.gov/business-services/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("south_dakota_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/south_dakota_businesses.json",
    "state_file":           "data/south_dakota_scraper_state.json",
...
class SouthDakotaScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new South Dakota businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique South Dakota businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("SOUTH DAKOTA SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "SD"])
        ...
        logger.info(f"  SD total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ South Dakota scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting South Dakota Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="South Dakota Business Scraper - 24h Scheduler")
    ...
    sc = SouthDakotaScheduledScraper(cfg)
