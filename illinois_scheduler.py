#!/usr/bin/env python3
"""
Illinois Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://apps.ilsos.gov/businessentitysearch/
  - Main:       https://www.ilsos.gov/departments/business_services/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("illinois_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/illinois_businesses.json",
    "state_file":           "data/illinois_scraper_state.json",
...
class IllinoisScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Illinois businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Illinois businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("ILLINOIS SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "IL"])
        ...
        logger.info(f"  IL total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Illinois scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Illinois Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Illinois Business Scraper - 24h Scheduler")
    ...
    sc = IllinoisScheduledScraper(cfg)
