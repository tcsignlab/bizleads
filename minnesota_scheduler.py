#!/usr/bin/env python3
"""
Minnesota Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://mblsportal.sos.state.mn.us/Business/Search
  - Main:       https://www.sos.state.mn.us/business-liens/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("minnesota_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/minnesota_businesses.json",
    "state_file":           "data/minnesota_scraper_state.json",
...
class MinnesotaScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Minnesota businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Minnesota businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("MINNESOTA SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "MN"])
        ...
        logger.info(f"  MN total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Minnesota scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Minnesota Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Minnesota Business Scraper - 24h Scheduler")
    ...
    sc = MinnesotaScheduledScraper(cfg)
