#!/usr/bin/env python3
"""
Oklahoma Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://www.sos.ok.gov/corp/corpInquiryFind.aspx
  - Main:       https://www.sos.ok.gov/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("oklahoma_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/oklahoma_businesses.json",
    "state_file":           "data/oklahoma_scraper_state.json",
...
class OklahomaScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Oklahoma businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Oklahoma businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("OKLAHOMA SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "OK"])
        ...
        logger.info(f"  OK total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Oklahoma scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Oklahoma Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Oklahoma Business Scraper - 24h Scheduler")
    ...
    sc = OklahomaScheduledScraper(cfg)
