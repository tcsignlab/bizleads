#!/usr/bin/env python3
"""
Delaware Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://icis.corp.delaware.gov/ecorp/entitysearch/namesearch.aspx
  - Main:       https://corp.delaware.gov/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("delaware_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/delaware_businesses.json",
    "state_file":           "data/delaware_scraper_state.json",
...
class DelawareScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Delaware businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Delaware businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("DELAWARE SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "DE"])
        ...
        logger.info(f"  DE total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Delaware scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Delaware Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Delaware Business Scraper - 24h Scheduler")
    ...
    sc = DelawareScheduledScraper(cfg)
