#!/usr/bin/env python3
"""
California Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://bizfileonline.sos.ca.gov/search/business
  - Main:       https://api.sos.ca.gov/business
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("california_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/california_businesses.json",
    "state_file":           "data/california_scraper_state.json",
...
class CaliforniaScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new California businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique California businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("CALIFORNIA SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "CA"])
        ...
        logger.info(f"  CA total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ California scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting California Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="California Business Scraper - 24h Scheduler")
    ...
    sc = CaliforniaScheduledScraper(cfg)
