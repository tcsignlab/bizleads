#!/usr/bin/env python3
"""
North Dakota Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://firststop.sos.nd.gov/search/business
  - Main:       https://sos.nd.gov/business
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("north_dakota_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/north_dakota_businesses.json",
    "state_file":           "data/north_dakota_scraper_state.json",
...
class NorthDakotaScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new North Dakota businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique North Dakota businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("NORTH DAKOTA SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "ND"])
        ...
        logger.info(f"  ND total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ North Dakota scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting North Dakota Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="North Dakota Business Scraper - 24h Scheduler")
    ...
    sc = NorthDakotaScheduledScraper(cfg)
