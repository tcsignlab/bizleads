#!/usr/bin/env python3
"""
New Jersey Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://www.njportal.com/DOR/BusinessRecords/
  - Main:       https://www.nj.gov/treasury/revenue/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("new_jersey_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/new_jersey_businesses.json",
    "state_file":           "data/new_jersey_scraper_state.json",
...
class NewJerseyScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new New Jersey businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique New Jersey businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("NEW JERSEY SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "NJ"])
        ...
        logger.info(f"  NJ total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ New Jersey scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting New Jersey Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="New Jersey Business Scraper - 24h Scheduler")
    ...
    sc = NewJerseyScheduledScraper(cfg)
