#!/usr/bin/env python3
"""
Nebraska Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://www.nebraska.gov/sos/corp/corpsearch.cgi
  - Main:       https://sos.nebraska.gov/business
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("nebraska_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/nebraska_businesses.json",
    "state_file":           "data/nebraska_scraper_state.json",
...
class NebraskaScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Nebraska businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Nebraska businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("NEBRASKA SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "NE"])
        ...
        logger.info(f"  NE total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Nebraska scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Nebraska Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Nebraska Business Scraper - 24h Scheduler")
    ...
    sc = NebraskaScheduledScraper(cfg)
