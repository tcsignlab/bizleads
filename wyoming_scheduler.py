#!/usr/bin/env python3
"""
Wyoming Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://wyobiz.wyo.gov/Business/FilingSearch.aspx
  - Main:       https://sos.wyo.gov/Business/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("wyoming_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/wyoming_businesses.json",
    "state_file":           "data/wyoming_scraper_state.json",
...
class WyomingScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Wyoming businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Wyoming businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("WYOMING SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "WY"])
        ...
        logger.info(f"  WY total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Wyoming scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Wyoming Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Wyoming Business Scraper - 24h Scheduler")
    ...
    sc = WyomingScheduledScraper(cfg)
