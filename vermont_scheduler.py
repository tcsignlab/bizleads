#!/usr/bin/env python3
"""
Vermont Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://bizfilings.vermont.gov/online/BusinessInquire/
  - Main:       https://sos.vermont.gov/corporations/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("vermont_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/vermont_businesses.json",
    "state_file":           "data/vermont_scraper_state.json",
...
class VermontScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Vermont businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Vermont businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("VERMONT SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "VT"])
        ...
        logger.info(f"  VT total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Vermont scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Vermont Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Vermont Business Scraper - 24h Scheduler")
    ...
    sc = VermontScheduledScraper(cfg)
