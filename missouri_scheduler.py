#!/usr/bin/env python3
"""
Missouri Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://bsd.sos.mo.gov/BusinessEntity/BESearch.aspx
  - Main:       https://www.sos.mo.gov/business
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("missouri_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/missouri_businesses.json",
    "state_file":           "data/missouri_scraper_state.json",
...
class MissouriScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Missouri businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Missouri businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("MISSOURI SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "MO"])
        ...
        logger.info(f"  MO total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Missouri scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Missouri Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Missouri Business Scraper - 24h Scheduler")
    ...
    sc = MissouriScheduledScraper(cfg)
