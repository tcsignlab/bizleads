#!/usr/bin/env python3
"""
Maine Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://icrs.informe.org/nei-sos-icrs/ICRS
  - Main:       https://www.maine.gov/sos/cec/corp/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("maine_scheduler.log"),
...
class MaineScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Maine businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Maine businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("MAINE SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "ME"])
        ...
        logger.info(f"  ME total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Maine scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Maine Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Maine Business Scraper - 24h Scheduler")
