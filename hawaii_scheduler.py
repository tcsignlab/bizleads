#!/usr/bin/env python3
"""
Hawaii Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://hbe.ehawaii.gov/documents/search.html
  - Main:       https://cca.hawaii.gov/breg/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("hawaii_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/hawaii_businesses.json",
    "state_file":           "data/hawaii_scraper_state.json",
...
class HawaiiScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Hawaii businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Hawaii businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("HAWAII SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "HI"])
        ...
        logger.info(f"  HI total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Hawaii scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Hawaii Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Hawaii Business Scraper - 24h Scheduler")
