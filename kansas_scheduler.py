#!/usr/bin/env python3
"""
Kansas Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://www.kansas.gov/bess/flow/main
  - Main:       https://sos.kansas.gov/business-services/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("kansas_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/kansas_businesses.json",
    "state_file":           "data/kansas_scraper_state.json",
...
class KansasScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Kansas businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Kansas businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("KANSAS SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "KS"])
        ...
        logger.info(f"  KS total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Kansas scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Kansas Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Kansas Business Scraper - 24h Scheduler")
