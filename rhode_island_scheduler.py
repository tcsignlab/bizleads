#!/usr/bin/env python3
"""
Rhode Island Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://business.sos.ri.gov/CorpWeb/CorpSearch/CorpSearch.aspx
  - Main:       https://www.sos.ri.gov/divisions/business-services
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("rhode_island_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/rhode_island_businesses.json",
    "state_file":           "data/rhode_island_scraper_state.json",
...
class RhodeIslandScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Rhode Island businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Rhode Island businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("RHODE ISLAND SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "RI"])
        ...
        logger.info(f"  RI total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Rhode Island scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Rhode Island Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Rhode Island Business Scraper - 24h Scheduler")
    ...
    sc = RhodeIslandScheduledScraper(cfg)
