#!/usr/bin/env python3
"""
Alaska Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://myalaska.state.ak.us/business
  - Main:       https://www.commerce.alaska.gov/cbp/main/search/entities
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("alaska_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/alaska_businesses.json",
    "state_file":           "data/alaska_scraper_state.json",
...
class AlaskaScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Alaska businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Alaska businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("ALASKA SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "AK"])
        ...
        logger.info(f"  AK total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Alaska scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Alaska Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Alaska Business Scraper - 24h Scheduler")
