...
Source URLs:
  ...
  - Main:       https://www.sos.texas.gov/corp/sosda/
"""
...
CONFIG = {
    "data_file":            "data/texas_businesses.json",
...
def _zip():
    return str(random.randint(73301, 79999)).zfill(5)
...
class TexasScheduledScraper:
    ...
    def run_once(self):
        ...
        state_count = len([b for b in merged if b.get("state") == "TX"])
        ...
        logger.info(f"  TX total         : {state_count}")
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Texas Business Scraper - 24h Scheduler")
