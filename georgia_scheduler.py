#!/usr/bin/env python3
"""
Georgia Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://ecorp.sos.ga.gov/BusinessSearch
  - Main:       https://sos.ga.gov/corporations
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("georgia_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/georgia_businesses.json",
    "state_file":           "data/georgia_scraper_state.json",
...
class GeorgiaScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Georgia businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Georgia businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("GEORGIA SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "GA"])
        ...
        logger.info(f"  GA total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Georgia scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Georgia Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Georgia Business Scraper - 24h Scheduler")
    ...
    sc = GeorgiaScheduledScraper(cfg)
