#!/usr/bin/env python3
"""
New York Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://appext20.dos.ny.gov/corp_public/corpsearch.entity_search_entry
  - Main:       https://dos.ny.gov/corporations-state-records-bureau
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("new_york_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/new_york_businesses.json",
    "state_file":           "data/new_york_scraper_state.json",
...
class NewYorkScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new New York businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique New York businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("NEW YORK SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "NY"])
        ...
        logger.info(f"  NY total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ New York scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting New York Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="New York Business Scraper - 24h Scheduler")
    ...
    sc = NewYorkScheduledScraper(cfg)
