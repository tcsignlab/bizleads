#!/usr/bin/env python3
"""
Utah Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://secure.utah.gov/bes/
  - Main:       https://corporations.utah.gov/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("utah_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/utah_businesses.json",
    "state_file":           "data/utah_scraper_state.json",
...
class UtahScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Utah businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Utah businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("UTAH SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "UT"])
        ...
        logger.info(f"  UT total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Utah scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Utah Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Utah Business Scraper - 24h Scheduler")
