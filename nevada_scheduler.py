#!/usr/bin/env python3
"""
Nevada Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://esos.nv.gov/EntitySearch/OnlineEntitySearch
  - Main:       https://www.nvsos.gov/sos
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("nevada_scheduler.log"),
...
class NevadaScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Nevada businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Nevada businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("NEVADA SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "NV"])
        ...
        logger.info(f"  NV total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Nevada scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Nevada Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Nevada Business Scraper - 24h Scheduler")
