#!/usr/bin/env python3
"""
West Virginia Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://apps.wv.gov/SOS/BusinessEntitySearch/
  - Main:       https://sos.wv.gov/business-licensing/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("west_virginia_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/west_virginia_businesses.json",
    "state_file":           "data/west_virginia_scraper_state.json",
...
class WestVirginiaScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new West Virginia businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique West Virginia businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("WEST VIRGINIA SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "WV"])
        ...
        logger.info(f"  WV total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ West Virginia scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting West Virginia Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="West Virginia Business Scraper - 24h Scheduler")
    ...
    sc = WestVirginiaScheduledScraper(cfg)
