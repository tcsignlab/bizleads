#!/usr/bin/env python3
"""
Louisiana Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://coraweb.sos.la.gov/CommercialSearch/CommercialSearch.aspx
  - Main:       https://www.sos.la.gov/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("louisiana_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/louisiana_businesses.json",
    "state_file":           "data/louisiana_scraper_state.json",
...
class LouisianaScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Louisiana businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Louisiana businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("LOUISIANA SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "LA"])
        ...
        logger.info(f"  LA total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Louisiana scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Louisiana Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Louisiana Business Scraper - 24h Scheduler")
    ...
    sc = LouisianaScheduledScraper(cfg)
