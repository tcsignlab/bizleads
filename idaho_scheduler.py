#!/usr/bin/env python3
"""
Idaho Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://sosbiz.idaho.gov/search/business
  - Main:       https://sos.idaho.gov/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("idaho_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/idaho_businesses.json",
    "state_file":           "data/idaho_scraper_state.json",
...
class IdahoScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Idaho businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Idaho businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("IDAHO SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "ID"])
        ...
        logger.info(f"  ID total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Idaho scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Idaho Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Idaho Business Scraper - 24h Scheduler")
