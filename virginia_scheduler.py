#!/usr/bin/env python3
"""
Virginia Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://cis.scc.virginia.gov/EntitySearch/Index
  - Main:       https://scc.virginia.gov/pages/businesses
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("virginia_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/virginia_businesses.json",
    "state_file":           "data/virginia_scraper_state.json",
...
class VirginiaScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Virginia businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Virginia businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("VIRGINIA SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "VA"])
        ...
        logger.info(f"  VA total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Virginia scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Virginia Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Virginia Business Scraper - 24h Scheduler")
    ...
    sc = VirginiaScheduledScraper(cfg)
