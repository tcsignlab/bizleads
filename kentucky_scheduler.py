#!/usr/bin/env python3
"""
Kentucky Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://web.sos.ky.gov/ftsearch/
  - Main:       https://sos.ky.gov/businesses
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("kentucky_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/kentucky_businesses.json",
    "state_file":           "data/kentucky_scraper_state.json",
...
class KentuckyScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Kentucky businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Kentucky businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("KENTUCKY SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "KY"])
        ...
        logger.info(f"  KY total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Kentucky scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Kentucky Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Kentucky Business Scraper - 24h Scheduler")
    ...
    sc = KentuckyScheduledScraper(cfg)
