#!/usr/bin/env python3
"""
Ohio Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://businesssearch.ohiosos.gov/
  - Main:       https://www.ohiosos.gov/businesses/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("ohio_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/ohio_businesses.json",
    "state_file":           "data/ohio_scraper_state.json",
...
class OhioScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Ohio businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Ohio businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("OHIO SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "OH"])
        ...
        logger.info(f"  OH total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Ohio scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Ohio Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Ohio Business Scraper - 24h Scheduler")
