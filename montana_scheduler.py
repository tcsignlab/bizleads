#!/usr/bin/env python3
"""
Montana Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://biz.sosmt.gov/search/business
  - Main:       https://sosmt.gov/business/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("montana_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/montana_businesses.json",
    "state_file":           "data/montana_scraper_state.json",
...
class MontanaScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Montana businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Montana businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("MONTANA SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "MT"])
        ...
        logger.info(f"  MT total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Montana scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Montana Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Montana Business Scraper - 24h Scheduler")
    ...
    sc = MontanaScheduledScraper(cfg)
