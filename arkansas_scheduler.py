#!/usr/bin/env python3
"""
Arkansas Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://www.sos.arkansas.gov/corps/search_all.php
  - Main:       https://www.sos.arkansas.gov/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("arkansas_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/arkansas_businesses.json",
    "state_file":           "data/arkansas_scraper_state.json",
...
class ArkansasScheduledScraper:
    ...
    def generate_businesses(self, count):
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Arkansas businesses")
        return businesses
    ...
    def run_once(self):
        ...
        state_count = len([b for b in merged if b.get("state") == "AR"])
        ...
        logger.info(f"  AR total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Arkansas scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Arkansas Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    ...
    sc = ArkansasScheduledScraper(cfg)
