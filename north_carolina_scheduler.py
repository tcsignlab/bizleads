...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("north_carolina_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/north_carolina_businesses.json",
    "state_file":           "data/north_carolina_scraper_state.json",
...
class NorthCarolinaScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new North Carolina businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique North Carolina businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("NORTH CAROLINA SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "NC"])
        ...
        logger.info(f"  NC total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ North Carolina scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting North Carolina Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="North Carolina Business Scraper - 24h Scheduler")
    ...
    sc = NorthCarolinaScheduledScraper(cfg)
