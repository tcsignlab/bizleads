#!/usr/bin/env python3
"""
Wisconsin Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://www.wdfi.org/apps/CorpSearch/
  - Main:       https://www.wdfi.org/corporations/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("wisconsin_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/wisconsin_businesses.json",
    "state_file":           "data/wisconsin_scraper_state.json",
...
class WisconsinScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Wisconsin businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Wisconsin businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("WISCONSIN SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "WI"])
        ...
        logger.info(f"  WI total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Wisconsin scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Wisconsin Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Wisconsin Business Scraper - 24h Scheduler")
    ...
    sc = WisconsinScheduledScraper(cfg)
