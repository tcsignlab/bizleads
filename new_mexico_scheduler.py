#!/usr/bin/env python3
"""
New Mexico Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://portal.sos.state.nm.us/BFS/online/CorporationBusinessSearch
  - Main:       https://www.sos.nm.gov/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("new_mexico_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/new_mexico_businesses.json",
    "state_file":           "data/new_mexico_scraper_state.json",
...
class NewMexicoScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new New Mexico businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique New Mexico businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("NEW MEXICO SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "NM"])
        ...
        logger.info(f"  NM total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ New Mexico scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting New Mexico Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="New Mexico Business Scraper - 24h Scheduler")
    ...
    sc = NewMexicoScheduledScraper(cfg)
