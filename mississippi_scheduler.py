#!/usr/bin/env python3
"""
Mississippi Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://business.sos.ms.gov/corp/soskb/csearch.asp
  - Main:       https://www.sos.ms.gov/business-services
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("mississippi_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/mississippi_businesses.json",
    "state_file":           "data/mississippi_scraper_state.json",
...
class MississippiScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Mississippi businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Mississippi businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("MISSISSIPPI SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "MS"])
        ...
        logger.info(f"  MS total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Mississippi scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Mississippi Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Mississippi Business Scraper - 24h Scheduler")
    ...
    sc = MississippiScheduledScraper(cfg)
