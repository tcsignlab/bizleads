#!/usr/bin/env python3
"""
Oregon Business Scraper - 24-Hour Scheduled Service
Source URLs:
  - SOS Search: https://egov.sos.state.or.us/br/pkg_web_name_srch_inq.login
  - Main:       https://sos.oregon.gov/business/
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("oregon_scheduler.log"),
...
CONFIG = {
    "data_file":            "data/oregon_businesses.json",
    "state_file":           "data/oregon_scraper_state.json",
...
class OregonScheduledScraper:
    ...
    def generate_businesses(self, count):
        logger.info(f"Generating {count} new Oregon businesses...")
        ...
        logger.info(f"✓ Generated {len(businesses)} unique Oregon businesses")
        return businesses
    ...
    def run_once(self):
        logger.info("=" * 60)
        logger.info("OREGON SCHEDULED SCRAPER - Starting run")
        logger.info("=" * 60)
        ...
        state_count = len([b for b in merged if b.get("state") == "OR"])
        ...
        logger.info(f"  OR total         : {state_count}")
        logger.info(f"  Run #            : {self.state['run_count']}")
        logger.info(f"  Next run         : {next_ts}")
        logger.info("")
        logger.info("✓ Oregon scraper run completed successfully")
        logger.info("=" * 60)

    def run_forever(self):
        logger.info("Starting Oregon Scheduled Scraper (24-hour mode)")
        while True:
            try:
                self.run_once()
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Oregon Business Scraper - 24h Scheduler")
